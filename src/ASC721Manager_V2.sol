 ...
 * - NFT_ROLE: is an IAnimalSocialClubERC721 contract address, and is the only
 *   role which can interact with the `lottery` contract.
 */
/// @custom:oz-upgrades-from ASC721Manager
contract ASC721Manager_V2 is
    ...
    UUPSUpgradeable
{
    ...
    IAnimalSocialClubERC721 public elephant;
    /// references to Tiger contract
    IAnimalSocialClubERC721 public tiger;
    /// references to Shark contract
    IAnimalSocialClubERC721 public shark;
    /// references to Eagle contract
    IAnimalSocialClubERC721 public eagle;
    /// references to Stakeholder contract
    IAnimalSocialClubERC721 public stakeholder;
     ...
     */
    IAnimalSocialClubERC721[] public contracts;
     ...
     * @dev Requires that all the argument are addresses of IAnimalSocialClubERC721 proxy contracts
     *
     * @param _elephant address of a deployed IAnimalSocialClubERC721 contract
     * @param _tiger address of a deployed IAnimalSocialClubERC721 contract
     * @param _shark address of a deployed IAnimalSocialClubERC721 contract
     * @param _eagle address of a deployed IAnimalSocialClubERC721 contract
     * @param _stakeholder address of a deployed IAnimalSocialClubERC721 contract
     */
    function assignContracts(
        ...
        elephant = IAnimalSocialClubERC721(_elephant);
        contracts.push(IAnimalSocialClubERC721(_elephant));
        require(_grantRole(NFT_ROLE, _elephant), "could not grant role");

        tiger = IAnimalSocialClubERC721(_tiger);
        contracts.push(IAnimalSocialClubERC721(_tiger));
        require(_grantRole(NFT_ROLE, _tiger), "could not grant role");

        shark = IAnimalSocialClubERC721(_shark);
        contracts.push(IAnimalSocialClubERC721(_shark));
        require(_grantRole(NFT_ROLE, _shark), "could not grant role");

        eagle = IAnimalSocialClubERC721(_eagle);
        contracts.push(IAnimalSocialClubERC721(_eagle));
        require(_grantRole(NFT_ROLE, _eagle), "could not grant role");

        stakeholder = IAnimalSocialClubERC721(_stakeholder);
        contracts.push(IAnimalSocialClubERC721(_stakeholder));
    ...
    function isMember(address a) public view returns (bool) {
        ...
        for (uint256 i = 0; i < len; i++) {
            IAnimalSocialClubERC721 tier = contracts[i];
     ...
     * function. See `IAnimalSocialClubERC721.adminMint`.
     * Only admin.
    ...
    function adminMint(
        ...
        IAnimalSocialClubERC721(contracts[tier]).adminMint(to);
    ...
    /**
     * @dev calls  `IAnimalSocialClubERC721.assignRole` on each sub-contract.
     * See that function.
     */
    function assignRole(
        ...
        for (uint256 i = 0; i < contracts.length; i++) {
            IAnimalSocialClubERC721 tier = contracts[i];
            // slither-disable-next-line calls-loop
            IVera3DistributionModel(tier).assignRole(
                upper,
                role,
                delegate,
                msg.sender
    ...
    /**
     * @dev calls  `IAnimalSocialClubERC721.setSaleActive`  on each sub-contract.
     */
