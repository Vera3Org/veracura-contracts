...
contract AddAdvocate is Script {
    function setUp() public {}

    function run() public {
        address admin = vm.envAddress("WALLET_ADDRESS");
        address MANAGER_PROXY = vm.envAddress("ASC_MANAGER");
        address payable AMBASSADOR_ADDRESS = payable(
            vm.envAddress("AMBASSADOR_ADDRESS")
        );
        address payable ADVOCATE_ADDRESS = payable(
            vm.envAddress("ADVOCATE_ADDRESS")
        );
